# Builds a json file of the bible structure

import json

books = [    
    {
        'testament': 'OT',
        'type': 'Pent',
        'verse_counts': [31, 25, 24, 26, 32, 22, 24, 22, 29, 32, 32, 20, 18, 24, 21, 16, 27, 33, 38, 18, 34, 24, 20, 67, 34, 35, 46, 22, 35, 43, 55, 32, 20, 31, 29, 43, 36, 30, 23, 23, 57, 38, 34, 34, 28, 34, 31, 22, 33, 26],
        'name': 'Genesis',
        'abbreviations': ['Gen', 'ge', 'gn'],
        'altname': 'The First Book of Moses, called Genesis',
    },
    {
        'testament': 'OT',
        'type': 'Pent',
        'verse_counts': [22, 25, 22, 31, 23, 30, 25, 32, 35, 29, 10, 51, 22, 31, 27, 36, 16, 27, 25, 26, 36, 31, 33, 18, 40, 37, 21, 43, 46, 38, 18, 35, 23, 35, 35, 38, 29, 31, 43, 38],
        'name': 'Exodus',
        'abbreviations': ['Exo', 'ex', 'exod'],
        'altname': 'The Second Book of Moses, called Exodus',
    },
    {
        'testament': 'OT',
        'type': 'Pent',
        'verse_counts': [17, 16, 17, 35, 19, 30, 38, 36, 24, 20, 47, 8, 59, 57, 33, 34, 16, 30, 37, 27, 24, 33, 44, 23, 55, 46, 34],
        'name': 'Leviticus',
        'abbreviations': ['Lev', 'lv', 'le'],
        'altname': 'The Third Book of Moses, called Leviticus',
    },
    {
        'testament': 'OT',
        'type': 'Pent',
        'verse_counts': [54, 34, 51, 49, 31, 27, 89, 26, 23, 36, 35, 16, 33, 45, 41, 50, 13, 32, 22, 29, 35, 41, 30, 25, 18, 65, 23, 31, 40, 16, 54, 42, 56, 29, 34, 13],
        'name': 'Numbers',
        'abbreviations': ['Num', 'nm', 'nu'],
        'altname': 'The Fourth Book of Moses, called Numbers',
    },
    {
        'testament': 'OT',
        'type': 'Pent',
        'verse_counts': [46, 37, 29, 49, 33, 25, 26, 20, 29, 22, 32, 32, 18, 29, 23, 22, 20, 22, 21, 20, 23, 30, 25, 22, 19, 19, 26, 68, 29, 20, 30, 52, 29, 12],
        'name': 'Deuteronomy',
        'abbreviations': ['Deu', 'deut', 'de', 'du', 'dt'],
        'altname': 'The Fifth Book of Moses, called Deuteronomy',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [18, 24, 17, 24, 15, 27, 26, 35, 27, 43, 23, 24, 33, 15, 63, 10, 18, 28, 51, 9, 45, 34, 16, 33],
        'name': 'Joshua',
        'abbreviations': ['Jos', 'josh'],
        'altname': 'The Book of Joshua',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [36, 23, 31, 24, 31, 40, 25, 35, 57, 18, 40, 15, 25, 20, 20, 31, 13, 31, 30, 48, 25],
        'name': 'Judges',
        'abbreviations': ['Jdg', 'jgs', 'judg'],
        'altname': 'The Book of Judges',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [22, 23, 18, 22],
        'name': 'Ruth',
        'abbreviations': ['Rut', 'ruth', 'ru'],
        'altname': 'The Book of Ruth',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [28, 36, 21, 22, 12, 21, 17, 22, 27, 27, 15, 25, 23, 52, 35, 23, 58, 30, 24, 42, 15, 23, 29, 22, 44, 25, 12, 25, 11, 31, 13],
        'name': '1 Samuel',
        'abbreviations': ['1Sa', '1 sam', '1sm', '1 sm', '1samuel', '1sam', '1 sa'],
        'altname': 'The First Book of Samuel, otherwise called the First Book of the Kings',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [27, 32, 39, 12, 25, 23, 29, 18, 13, 19, 27, 31, 39, 33, 37, 23, 29, 33, 43, 26, 22, 51, 39, 25],
        'name': '2 Samuel',
        'abbreviations': ['2Sa', '2 sam', '2sm', '2 sm', '2samuel', '2sam', '2 sa'],
        'altname': 'The Second Book of Samuel, otherwise called the Second Book of the Kings',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [53, 46, 28, 34, 18, 38, 51, 66, 28, 29, 43, 33, 34, 31, 34, 34, 24, 46, 21, 43, 29, 53],
        'name': '1 Kings',
        'abbreviations': ['1Ki', '1kg', '1 kg', '1kings', '1king', '1 ki'],
        'altname': 'The First Book of the Kings, commonly called the Third Book of the Kings',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [18, 25, 27, 44, 27, 33, 20, 29, 37, 36, 21, 21, 25, 29, 38, 20, 41, 37, 37, 21, 26, 20, 37, 20, 30],
        'name': '2 Kings',
        'abbreviations': ['2Ki', '2kg 2', 'kg', '2kings', '2king', '2 ki'],
        'altname': 'The Second Book of the Kings, commonly called the Fourth Book of the Kings',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [54, 55, 24, 43, 26, 81, 40, 40, 44, 14, 47, 40, 14, 17, 29, 43, 27, 17, 19, 8, 30, 19, 32, 31, 31, 32, 34, 21, 30],
        'name': '1 Chronicles',
        'abbreviations': ['1Ch', '1chronicles', '1ch', '1 chron', '1 chron'],
        'altname': 'The First Book of the Chronicles',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [17, 18, 17, 22, 14, 42, 22, 18, 31, 19, 23, 16, 22, 15, 19, 14, 19, 34, 11, 37, 20, 12, 21, 27, 28, 23, 9, 27, 36, 27, 21, 33, 25, 33, 27, 23],
        'name': '2 Chronicles',
        'abbreviations': ['2Ch', '2chronicles', '2ch', '2 chron', '2chron'],
        'altname': 'The Second Book of the Chronicles',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [11, 70, 13, 24, 17, 22, 28, 36, 15, 44],
        'name': 'Ezra',
        'abbreviations': ['Ezr', 'ez'],
        'altname': 'Ezra',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [11, 20, 32, 23, 19, 19, 73, 18, 38, 39, 36, 47, 31],
        'name': 'Nehemiah',
        'abbreviations': ['Neh', 'ne', 'nehem'],
        'altname': 'The Book of Nehemiah',
    },
    {
        'testament': 'OT',
        'type': 'Hist',
        'verse_counts': [22, 23, 15, 17, 14, 14, 10, 17, 32, 3],
        'name': 'Esther',
        'abbreviations': ['Est', 'es', 'esth'],
        'altname': 'The Book of Esther',
    },
    {
        'testament': 'OT',
        'type': 'Poet',
        'verse_counts': [22, 13, 26, 21, 27, 30, 21, 22, 35, 22, 20, 25, 28, 22, 35, 22, 16, 21, 29, 29, 34, 30, 17, 25, 6, 14, 23, 28, 25, 31, 40, 22, 33, 37, 16, 33, 24, 41, 30, 24, 34, 17],
        'name': 'Job',
        'abbreviations': ['Job', 'jb'],
        'altname': 'The Book of Job',
    },
    {
        'testament': 'OT',
        'type': 'Poet',
        'verse_counts': [6, 12, 8, 8, 12, 10, 17, 9, 20, 18, 7, 8, 6, 7, 5, 11, 15, 50, 14, 9, 13, 31, 6, 10, 22, 12, 14, 9, 11, 12, 24, 11, 22, 22, 28, 12, 40, 22, 13, 17, 13, 11, 5, 26, 17, 11, 9, 14, 20, 23, 19, 9, 6, 7, 23, 13, 11, 11, 17, 12, 8, 12, 11, 10, 13, 20, 7, 35, 36, 5, 24, 20, 28, 23, 10, 12, 20, 72, 13, 19, 16, 8, 18, 12, 13, 17, 7, 18, 52, 17, 16, 15, 5, 23, 11, 13, 12, 9, 9, 5, 8, 28, 22, 35, 45, 48, 43, 13, 31, 7, 10, 10, 9, 8, 18, 19, 2, 29, 176, 7, 8, 9, 4, 8, 5, 6, 5, 6, 8, 8, 3, 18, 3, 3, 21, 26, 9, 8, 24, 13, 10, 7, 12, 15, 21, 10, 20, 14, 9, 6],
        'name': 'Psalms',
        'abbreviations': ['Psa', 'pss', 'psalm', 'ps'],
        'altname': 'The Book of Psalms',
    },
    {
        'testament': 'OT',
        'type': 'Poet',
        'verse_counts': [33, 22, 35, 27, 23, 35, 27, 36, 18, 32, 31, 28, 25, 35, 33, 33, 28, 24, 29, 30, 31, 29, 35, 34, 28, 28, 27, 28, 27, 33, 31],
        'name': 'Proverbs',
        'abbreviations': ['Prv', 'prv', 'pv', 'prov'],
        'altname': 'The Proverbs',
    },
    {
        'testament': 'OT',
        'type': 'Poet',
        'verse_counts': [18, 26, 22, 16, 20, 12, 29, 17, 18, 20, 10, 14],
        'name': 'Ecclesiastes',
        'abbreviations': ['Ecc', 'ec', 'eccles'],
        'altname': 'Ecclesiastes, or, The Preacher',
    },
    {
        'testament': 'OT',
        'type': 'Poet',
        'verse_counts': [17, 17, 11, 16, 16, 13, 13, 14],
        'name': 'Song of Solomon',
        'abbreviations': ['Song', 'ss', 'so', 'sg', 'son', 'song of sol', 'sos'],
        'altname': 'The Song of Solomon',
    },
    {
        'testament': 'OT',
        'type': 'Major',
        'verse_counts': [31, 22, 26, 6, 30, 13, 25, 22, 21, 34, 16, 6, 22, 32, 9, 14, 14, 7, 25, 6, 17, 25, 18, 23, 12, 21, 13, 29, 24, 33, 9, 20, 24, 17, 10, 22, 38, 22, 8, 31, 29, 25, 28, 28, 25, 13, 15, 22, 26, 11, 23, 15, 12, 17, 13, 12, 21, 14, 21, 22, 11, 12, 19, 12, 25, 24],
        'name': 'Isaiah',
        'abbreviations': ['Isa', 'is'],
        'altname': 'The Book of the Prophet Isaiah',
    },
    {
        'testament': 'OT',
        'type': 'Major',
        'verse_counts': [19, 37, 25, 31, 31, 30, 34, 22, 26, 25, 23, 17, 27, 22, 21, 21, 27, 23, 15, 18, 14, 30, 40, 10, 38, 24, 22, 17, 32, 24, 40, 44, 26, 22, 19, 32, 21, 28, 18, 16, 18, 22, 13, 30, 5, 28, 7, 47, 39, 46, 64, 34],
        'name': 'Jeremiah',
        'abbreviations': ['Jer', 'je', 'jerem'],
        'altname': 'The Book of the Prophet Jeremiah',
    },
    {
        'testament': 'OT',
        'type': 'Major',
        'verse_counts': [22, 22, 66, 22, 22],
        'name': 'Lamentations',
        'abbreviations': ['Lam', 'la', 'lamen'],
        'altname': 'The Lamentations of Jeremiah',
    },
    {
        'testament': 'OT',
        'type': 'Major',
        'verse_counts': [28, 10, 27, 17, 17, 14, 27, 18, 11, 22, 25, 28, 23, 23, 8, 63, 24, 32, 14, 49, 32, 31, 49, 27, 17, 21, 36, 26, 21, 26, 18, 32, 33, 31, 15, 38, 28, 23, 29, 49, 26, 20, 27, 31, 25, 24, 23, 35],
        'name': 'Ezekiel',
        'abbreviations': ['Eze', 'ez', 'ezek', 'ezk'],
        'altname': 'The Book of the Prophet Ezekiel',
    },
    {
        'testament': 'OT',
        'type': 'Major',
        'verse_counts': [21, 49, 30, 37, 31, 28, 28, 27, 27, 21, 45, 13],
        'name': 'Daniel',
        'abbreviations': ['Dan', 'da', 'dn'],
        'altname': 'The Book of Daniel',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [11, 23, 5, 19, 15, 11, 16, 14, 17, 15, 12, 14, 16, 9],
        'name': 'Hosea',
        'abbreviations': ['Hos', 'ho'],
        'altname': 'Hosea',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [20, 32, 21],
        'name': 'Joel',
        'abbreviations': ['Joe', 'jl', 'joel'],
        'altname': 'Joel',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [15, 16, 15, 13, 27, 14, 17, 14, 15],
        'name': 'Amos',
        'abbreviations': ['Amo', 'am', 'amos'],
        'altname': 'Amos',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [21],
        'name': 'Obadiah',
        'abbreviations': ['Oba', 'ob', 'obad'],
        'altname': 'Obadiah',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [17, 10, 10, 11],
        'name': 'Jonah',
        'abbreviations': ['Jon', 'jonah', 'jnh'],
        'altname': 'Jonah',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [16, 13, 12, 13, 15, 16, 20],
        'name': 'Micah',
        'abbreviations': ['Mic', 'mi', 'micah'],
        'altname': 'Micah',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [15, 13, 19],
        'name': 'Nahum',
        'abbreviations': ['Nah', 'na'],
        'altname': 'Nahum',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [17, 20, 19],
        'name': 'Habakkuk',
        'abbreviations': ['Hab', 'hb'],
        'altname': 'Habakkuk',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [18, 15, 20],
        'name': 'Zephaniah',
        'abbreviations': ['Zep', 'zeph'],
        'altname': 'Zephaniah',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [15, 23],
        'name': 'Haggai',
        'abbreviations': ['Hag', 'hg'],
        'altname': 'Haggai',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [21, 13, 10, 14, 11, 15, 14, 23, 17, 12, 17, 14, 9, 21],
        'name': 'Zechariah',
        'abbreviations': ['Zec', 'zech'],
        'altname': 'Zechariah',
    },
    {
        'testament': 'OT',
        'type': 'Minor',
        'verse_counts': [14, 17, 18, 6],
        'name': 'Malachi',
        'abbreviations': ['Mal', 'ml'],
        'altname': 'Malachi',
    },
    {
        'testament': 'NT',
        'type': 'Gosp',
        'verse_counts': [25, 23, 17, 25, 48, 34, 29, 34, 38, 42, 30, 50, 58, 36, 39, 28, 27, 35, 30, 34, 46, 46, 39, 51, 46, 75, 66, 20],
        'name': 'Matthew',
        'abbreviations': ['Mat', 'matthew', 'matt', 'mt'],
        'altname': 'The Gospel According to St. Matthew',
        'shortname': 'Matthew',
    },
    {
        'testament': 'NT',
        'type': 'Gosp',
        'verse_counts': [45, 28, 35, 41, 43, 56, 37, 38, 50, 52, 33, 44, 37, 72, 47, 20],
        'name': 'Mark',
        'abbreviations': ['Mar', 'mark', 'mk'],
        'altname': 'The Gospel According to St. Mark',
        'shortname': 'Mark',
    },
    {
        'testament': 'NT',
        'type': 'Gosp',
        'verse_counts': [80, 52, 38, 44, 39, 49, 50, 56, 62, 42, 54, 59, 35, 35, 32, 31, 37, 43, 48, 47, 38, 71, 56, 53],
        'name': 'Luke',
        'abbreviations': ['Luk', 'lu', 'luke', 'lk'],
        'altname': 'The Gospel According to St. Luke',
        'shortname': 'Luke',
    },
    {
        'testament': 'NT',
        'type': 'Gosp',
        'verse_counts': [51, 25, 36, 54, 47, 71, 53, 59, 41, 42, 57, 50, 38, 31, 27, 33, 26, 40, 42, 31, 25],
        'name': 'John',
        'abbreviations': ['Joh', 'jo', 'john', 'jn'],
        'altname': 'The Gospel According to St. John',
        'shortname': 'John',
    },
    {
        'testament': 'NT',
        'type': 'Hist',
        'verse_counts': [26, 47, 26, 37, 42, 15, 60, 40, 43, 48, 30, 25, 52, 28, 41, 40, 34, 28, 41, 38, 40, 30, 35, 27, 27, 32, 44, 31],
        'name': 'Acts',
        'abbreviations': ['Act', 'ac', 'acts'],
        'altname': 'The Acts of the Apostles',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [32, 29, 31, 25, 21, 23, 25, 39, 33, 21, 36, 21, 14, 23, 33, 27],
        'name': 'Romans',
        'abbreviations': ['Rom', 'ro', 'rm'],
        'altname': 'The Epistle of Paul the Apostle to the Romans',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [31, 16, 23, 21, 13, 20, 40, 13, 27, 33, 34, 31, 13, 40, 58, 24],
        'name': '1 Corinthians',
        'abbreviations': ['1Co', '1c', '1corinthians', '1 co', '1cor', '1 cor'],
        'altname': 'The First Epistle of Paul the Apostle to the Corinthians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [24, 17, 18, 18, 21, 18, 16, 24, 15, 18, 33, 21, 14],
        'name': '2 Corinthians',
        'abbreviations': ['2Co', '2c', '2corinthians', '2 co', '2cor', '2 cor'],
        'altname': 'The Second Epistle of Paul the Apostle to the Corinthians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [24, 21, 29, 31, 26, 18],
        'name': 'Galatians',
        'abbreviations': ['Gal', 'ga'],
        'altname': 'The Epistle of Paul the Apostle to the Galatians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [23, 22, 21, 32, 33, 24],
        'name': 'Ephesians',
        'abbreviations': ['Eph', 'ep'],
        'altname': 'The Epistle of Paul the Apostle to the Ephesians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [30, 30, 21, 23],
        'name': 'Philippians',
        'abbreviations': ['Phi', 'php', 'phil'],
        'altname': 'The Epistle of Paul the Apostle to the Philippians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [29, 23, 25, 18],
        'name': 'Colossians',
        'abbreviations': ['Col', 'co'],
        'altname': 'The Epistle of Paul the Apostle to the Colossians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [10, 20, 13, 18, 28],
        'name': '1 Thessalonians',
        'abbreviations': ['1Th', '1thessalonians', '1thess', '1th', '1 thess', '1 thes', '1 th'],
        'altname': 'The First Epistle of Paul the Apostle to the Thessalonians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [12, 17, 18],
        'name': '2 Thessalonians',
        'abbreviations': ['2Th', '2thessalonians', '2thess', '2th', '2 thess', '2 thes', '2 th'],
        'altname': 'The Second Epistle of Paul the Apostle to the Thessalonians',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [20, 15, 16, 16, 25, 21],
        'name': '1 Timothy',
        'abbreviations': ['1Ti', '1tm', '1 tm', '1timothy', '1ti', '1 tim', '1 ti'],
        'altname': 'The First Epistle of Paul the Apostle to Timothy',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [18, 26, 17, 22],
        'name': '2 Timothy',
        'abbreviations': ['2Ti', '2tm', '2 tm', '2timothy', '2ti', '2 tim', '2 ti'],
        'altname': 'The Second Epistle of Paul the Apostle to Timothy',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [16, 15, 15],
        'name': 'Titus',
        'abbreviations': ['Tit', 'ti', 'titus'],
        'altname': 'The Epistle of Paul to Titus',
    },
    {
        'testament': 'NT',
        'type': 'Paul',
        'verse_counts': [25],
        'name': 'Philemon',
        'abbreviations': ['Phm', 'philem'],
        'altname': 'The Epistle of Paul to Philemon',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [14, 18, 19, 16, 14, 20, 28, 13, 28, 39, 40, 29, 25],
        'name': 'Hebrews',
        'abbreviations': ['Heb', 'he'],
        'altname': 'The Epistle of Paul the Apostle to the Hebrews',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [27, 26, 18, 17, 20],
        'name': 'James',
        'abbreviations': ['Jam', 'ja', 'jas'],
        'altname': 'The General Epistle of James',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [25, 25, 22, 19, 14],
        'name': '1 Peter',
        'abbreviations': ['1Pe', '1p', '1pe', '1 pe', '1pt', '1pe', '1 pet', '1 pt', '1 pe'],
        'altname': 'The First Epistle General of Peter',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [21, 22, 18],
        'name': '2 Peter',
        'abbreviations': ['2Pe', '2p', '2pe', '2 pe', '2pt', '2pe', '2 pet', '2 pt', '2 pe'],
        'altname': 'The Second Epistle General of Peter',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [10, 29, 24, 21, 21],
        'name': '1 John',
        'abbreviations': ['1Jo', '1j', '1jo', '1 jo', '1jn', '1 jn'],
        'altname': 'The First Epistle General of John',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [13],
        'name': '2 John',
        'abbreviations': ['2Jo', '2j', '2jo', '2 jo', '2jn', '2 jn'],
        'altname': 'The Second Epistle of John',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [14],
        'name': '3 John',
        'abbreviations': ['3Jo', '3j', '3jo', '3 jo', '3jn', '3 jn'],
        'altname': 'The Third Epistle of John',
    },
    {
        'testament': 'NT',
        'type': 'Epistle',
        'verse_counts': [25],
        'name': 'Jude',
        'abbreviations': ['Jud','ju', 'jude'],
        'altname': 'The General Epistle of Jude',
    },
    {
        'testament': 'NT',
        'type': 'Apoc',
        'verse_counts': [20, 29, 22, 11, 14, 17, 17, 13, 21, 11, 19, 17, 18, 20, 8, 21, 18, 24, 21, 15, 27, 21],
        'name': 'Revelation',
        'abbreviations': ['Rev', 're', 'rv', 'revel'],
        'altname': 'The Revelation of St. John the Divine',
    }
]

bible_structure = [{'idx': 'Bible', 'parent': '', 'name': 'Bible', 'size': 0, 'grpsize': 0}, {'idx': 'OT', 'name': 'OT', 'parent': 'Bible', 'size': 0, 'grpsize': 0}, {'idx': 'NT', 'name': 'NT', 'parent': 'Bible', 'size': 0, 'grpsize': 0}]
typeID = ''

for book in books:

  book_name = book['abbreviations'][0]

  if typeID != book['testament'] + book['type']: # only record "type" once for each "type"
    typeID = book['testament'] + book['type']
    bible_structure.append({'idx': typeID, 'name': book['type'], 'parent': book['testament'], 'size': 0, 'grpsize': 0})
    
  bible_structure.append({'idx': 'book_' + book_name, 'name': book_name, 'parent': typeID, 'verse_counts': book['verse_counts'], 'size': 1, 'grpsize': len(book['verse_counts'])})

  # INFO: unremark next 3 lines to get chapters
  #for chapter, verse_count in enumerate(book['verse_counts']): # chapter is really a 0-base 'position in array'
  #  ch_num = str(chapter + 1)
  #  bible_structure.append({'name': book_name + ' ' + ch_num, 'parent': book_name, 'verse_count': verse_count})

    # INFO: unremark next 2 lines to get verses (within chapters)
    #for v in range(verse_count): # is v 0-base?
    #  bible_structure.append({'name': book_name + ' ' + ch_num + ':' + str(v + 1), 'parent': book_name + ' ' + ch_num})

# Save as json
with open(r'working\bible_structure.json', 'w') as outfile:
    bible_json = json.dump(bible_structure, outfile)

a = 5
